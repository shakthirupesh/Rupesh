...
CREATE TABLE Employees (
    ...
    ALTER TABLE employees add FOREIGN KEY(branch_no) REFERENCES branch
    (branch_no) ON UPDATE CASCADE;
   
   ALTER TABLE employees add FOREIGN KEY(mngr_no) REFERENCES employees
    (emp_no) ON UPDATE CASCADE;

-- Create the Customers table with constraints
...
-- Create the Accounts table with constraints
CREATE TABLE Accounts (
    Acc_no INT AUTO_INCREMENT PRIMARY KEY,
    Branch_no INT,
    Cust_no INT,
    Type VARCHAR(2) CHECK (Type IN ('CA', 'SA')),
    OpnDT DATE,
    CurBal DECIMAL(10, 2),
    Status VARCHAR(255) CHECK (Status IN ('active', 'suspended', 'terminated')),
...
INSERT INTO Customers (Fname, Mname, Lname, City, DoB, Occupation) VALUES
    ('Ramesh', 'Chandra', 'Sharma', 'Service', '1976-12-06', 'Service'),
    ('Avinash', 'Sunder', 'Minha', 'Business', '1974-10-16', 'Business');

-- Insert records into Accounts table
INSERT INTO Accounts (Branch_no, Cust_no, Type, OpnDT, CurBal, Status) VALUES
    (1, 1, 'SA', '2012-12-15', 10000.00, 'Active'),
    (2, 2, 'SA', '2012-06-12', 5000.00, 'Active');
desc employees;
-- Insert records into Employees table
INSERT INTO Employees (Branch_no, Fname, Mname, Lname, Dept, Desig, Mngr_no) VALUES
    (1, 'Mark', 'Steve', 'Lara', 'Account', 'Accountant', 2),
...
ALTER TABLE Branch ADD City VARCHAR(255);
...
SELECT fname,curbal AS LowestBalance FROM Accounts a
join customers c on c.Cust_no=a.Cust_no
...
SELECT Fname, Lname FROM Employees WHERE Emp_no IN 
(SELECT Mngr_no FROM Employees);
...
SELECT * FROM Employees WHERE Dept IN ('loan', 'credit');

#15.Write a query to display the customer number, customer firstname, account number for the 
SELECT Customers.Cust_no, Customers.Fname, Accounts.Acc_no
FROM Customers
INNER JOIN Accounts ON Customers.Cust_no = Accounts.Cust_no;
...
SELECT
    ...
    A.Branch_no AS Branch_ID,
    A.CurBal AS Balance_Amount
FROM Customers  C
JOIN
Accounts A ON C.cust_no = A.cust_no;

#17.Create a virtual table to store the customers who are having the accounts in the same city as they live
CREATE VIEW CustomersWithAccountsInSameCity AS
SELECT C.cust_no,C.fname,C.mname,C.lname,C.city,A.Acc_no,A.Branch_no
FROM Customers AS C
JOIN
Accounts AS A ON C.cust_no = A.cust_no AND C.city = (SELECT City FROM Branch WHERE Branch_no = A.Branch_no);
...
CREATE TABLE Transactions (
    ...
    type VARCHAR(10),
    DOT DATE,
    FOREIGN KEY (Cust_id) REFERENCES Customers(cust_no),
...
BEGIN
    ...
    IF NEW.type = 'Deposit' THEN
        SET new_balance = (SELECT CurBal FROM Accounts WHERE Acc_no = NEW.account_no) + NEW.amount;
    ELSEIF NEW.type = 'Withdraw' THEN
        SET new_balance = (SELECT CurBal FROM Accounts WHERE Acc_no = NEW.account_no) - NEW.amount;
    END IF;
    
    UPDATE Accounts
    SET CurBal = new_balance
...
INSERT INTO Transactions (Cust_id, account_no, bid, amount, type, DOT)
VALUES (1, 1, 1, 1000.00, 'Deposit', '2023-09-07');

-- Insert a withdrawal transaction

INSERT INTO Transactions (Cust_id, account_no, bid, amount, type, DOT)
VALUES (2, 2, 2, 500.00, 'Withdraw', '2023-09-07');

#19.	Write a query to display the details of customer with second highest balance 
SELECT c.cust_no,c.fname,c.mname,c.lname,a.CurBal AS Balance
FROM Customers c
JOIN Accounts a ON c.cust_no = a.cust_no
WHERE a.CurBal =
(SELECT DISTINCT CurBal FROM Accounts ORDER BY CurBal DESC LIMIT 1 OFFSET 1);

#20.	Take backup of the databse created in this case study

#mysqldump -u root -p BANK > backup_file.sql
...
SELECT
CASE
WHEN product_class_code= 2050 THEN product_desc
WHEN product_class_code = 2051 THEN product_desc
WHEN product_class_code = 2052 THEN product_desc
END AS product_name,
CASE
WHEN product_class_code  = 2050 THEN product_price + 2000
WHEN product_class_code  = 2051 THEN product_price+ 500
WHEN product_class_code  = 2052 THEN product_price + 600
END AS adjusted_price,product_class_code
FROM product
...
SELECT pc.product_class_desc AS product_description,p.product_desc AS class_description,p.product_price
FROM product p
JOIN product_class pc ON p.product_class_code = pc.product_class_code
join 
order_items o on p.product_id= o.product_id
join 
order_header oh on o.order_id=o.order_id
...
#b. For Stationery and Clothes categories, if qty < 20, show 'Low stock', 21 < qty < 80, show 'In stock',
...
SELECT product_class_desc,p.product_class_code,product_quantity_avail,
CASE
WHEN (p.product_class_code IN (2050, 2051) AND product_quantity_avail  < 10) OR
	 (p.product_class_code IN (2001, 2002) AND product_quantity_avail < 10) OR
	 (p.product_class_code IN (2003, 2004) AND product_quantity_avail < 15) THEN 'Low stock'
WHEN (p.product_class_code IN (2050, 2051) AND product_quantity_avail BETWEEN 11 AND 30) OR
	 (p.product_class_code IN (2001, 2002) AND product_quantity_avail BETWEEN 11 AND 30) OR
	 (p.product_class_code IN (2003, 2004) AND product_quantity_avail BETWEEN 16 AND 50) THEN 'In stock'
WHEN (p.product_class_code IN (2050, 2051) AND product_quantity_avail > 31) OR
	 (p.product_class_code IN (2001, 2002) AND product_quantity_avail > 31) OR
	 (p.product_class_code IN (2003, 2004) AND product_quantity_avail > 50) THEN 'Enough stock'
ELSE 'Out of stock'
...
select distinct c.customer_fname,a.state,p.product_class_code,pc.product_class_desc
...
and  p.product_class_code in(2051,2054);
